...
describe("ApiService", () => {
  ...
  const mockResponse = (status: number, responseData: unknown) =>
    Promise.resolve({
  ...
  it("removeProduct should make a DELETE request to the correct URL", async () => {
    global.fetch = jest.fn().mockImplementation(() => mockResponse(200, {}));
  ...
  it("removeProduct should throw an error when the request fails", async () => {
    global.fetch = jest.fn().mockImplementation(() => mockResponse(400, {}));
  ...
  it("updateProduct should make a PUT request to the correct URL with the provided data", async () => {
    ...
    await ApiService.updateProduct("1", updatedProduct);
  ...
  it("updateProduct should throw an error when the request fails", async () => {
    ...
    await expect(
      ApiService.updateProduct("1", {} as CustomProductProps)
    ).rejects.toThrow(ERROR_MESSAGES.FETCH);
  });
  ...
  it("addProduct should throw an error when the request fails", async () => {
    ...
    await expect(
      ApiService.addProduct({} as CustomProductProps)
    ).rejects.toThrow(ERROR_MESSAGES.FETCH);
  });
